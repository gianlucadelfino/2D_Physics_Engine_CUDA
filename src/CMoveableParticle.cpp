...
CMoveableParticle::CMoveableParticle(const C2DVector& initial_pos_) : IMoveable(initial_pos_) {}

CMoveableParticle::CMoveableParticle(const CMoveableParticle& other_) : IMoveable(other_) {}
...
{
  return std::make_unique<CMoveableParticle>(*this);
}
