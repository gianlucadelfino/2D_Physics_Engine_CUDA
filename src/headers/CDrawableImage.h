...
public:
    CDrawableImage(CSurface* surf_to_blit_, SDL_Surface* destination_surf_)
        : IDrawable(destination_surf_), mp_surface(surf_to_blit_)
    {
    }

    CDrawableImage(const CDrawableImage& other_)
        : IDrawable(other_.mp_destination), mp_surface(other_.mp_surface)
    {
    }
    ...
    {
        this->mp_surface->ApplySurface(static_cast<int>(pos.x),
