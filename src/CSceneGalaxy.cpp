	...
	m_font(  new CFont( "pcseniorSmall.ttf", 20 ) )
{
	...
	{
		this->m_use_CUDA = false;
		this->m_stars_num = 1024;
		//add warning label
		//stars number label
		Uint32 black_color = SDL_MapRGB( this->mp_screen->format, 0, 0, 0 );
		SDL_Color label_color = { 255, 255, 255, 0 };
		std::unique_ptr< CDrawableButton > stars_num_label_drawable( new CDrawableButton( this->m_font, this->mp_screen, "CUDA compatible device not found, using CPU!", C2DVector( 220.0f, 22.0f ), black_color, label_color ) );
		std::unique_ptr< CMoveableButton > stars_num_label_moveable( new CMoveableButton( C2DVector( 700.0f, 20.0f ), C2DVector( 220.0f, 22.0f ) ) );
	...
	//create galaxy
	std::unique_ptr< CEntity > galaxy( new CEntityGalaxy( 2, C2DVector( 600.0f, 300.0f ), star, this->m_stars_num, 500, this->m_use_CUDA ) ); // id, pos, star prototype, stars number (max 1024*9), size
...
void CSceneGalaxy::Init()
{
	...
	unsigned int starting_stars_num = this->m_use_CUDA? 1024 : 8*1024; //start from less stars if we are switching to CPU
	std::unique_ptr< CEntity > CUDA_CPU_switch_button( new CEntityButton(
		...
		this->mr_world,
		std::move( std::unique_ptr< IScene >( new CSceneGalaxy( this->mp_screen, this->mr_world, !this->m_use_CUDA, starting_stars_num ) ) )
		));
	...
	std::unique_ptr< CEntity > more_stars_button( new CEntityButton(
		...
		this->mr_world,
		std::move( std::unique_ptr< IScene >( new CSceneGalaxy( this->mp_screen, this->mr_world, this->m_use_CUDA, this->m_stars_num + 1024 ) ) )
		));
