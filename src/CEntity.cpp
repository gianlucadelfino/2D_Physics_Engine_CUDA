...
CEntity::CEntity(unsigned int id_,
    ...
    : m_id(id_),
      mp_moveable(
          std::move(moveable_)), // transfer ownership with move semantics
      mp_drawable(
          std::move(drawable_)) // transfer ownership with move semantics
{
}

CEntity::CEntity(const CEntity& other_) : m_id(other_.m_id)
{
...
CEntity& CEntity::operator=(const CEntity& rhs)
{
    if (this != &rhs)
    {
        ...
        if (rhs.mp_moveable)
        {
            ...
            // IMovable!
            this->mp_moveable = std::move(rhs.mp_moveable->Clone());
        }
        else
            this->mp_moveable = NULL;
...
void CEntity::Draw() const
{
    if (this->mp_drawable && this->mp_moveable)
        mp_drawable->Draw(this->mp_moveable->pos,
                          this->mp_moveable->orientation);
}
