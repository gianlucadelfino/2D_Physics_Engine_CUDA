...
void CSceneMainMenu::Init()
{
    ...
    std::unique_ptr<CMoveableButton> title_moveable(new CMoveableButton(
        C2DVector(400.0f, 50.0f), C2DVector(300.0f, 31.0f)));
    std::unique_ptr<CDrawableButton> title_drawable(
        new CDrawableButton(title_font,
                            this->mp_screen,
                            "SIMULINO 2000",
                            C2DVector(200.0f, 31.0f),
                            black_color,
                            title_color));

    std::unique_ptr<CEntity> title(
        new CEntity(1, std::move(title_moveable), std::move(title_drawable)));
    ...
    std::unique_ptr<CMoveableButton> cloth_moveable_button(new CMoveableButton(
        C2DVector(500.0f, 300.0f), C2DVector(200.0f, 22.0f)));
    std::unique_ptr<CDrawableButton> cloth_button_drawable(
        new CDrawableButton(button_font,
                            this->mp_screen,
                            "CLOTH",
    ...
    std::unique_ptr<CEntity> cloth_sim_switch_button(
        new CEntityButton(1,
                          ...
                          *this->mr_world,
                          std::move(std::unique_ptr<IScene>(new CSceneCloth(
    ...
    std::unique_ptr<CMoveableButton> moveable_button(new CMoveableButton(
        C2DVector(500.0f, 400.0f), C2DVector(200.0f, 22.0f)));
    std::unique_ptr<CDrawableButton> galaxy_button_drawable(
        new CDrawableButton(button_font,
                            this->mp_screen,
                            "GALAXY",
                            C2DVector(200.0f, 22.0f),
    ...
    std::unique_ptr<CEntity> galaxy_sim_switch_button(
        new CEntityButton(1,
    ...
    // credits label
    std::unique_ptr<CMoveableButton> credits_moveable(new CMoveableButton(
        C2DVector(800.0f, 660.0f), C2DVector(300.0f, 22.0f)));
    ...
    std::unique_ptr<CEntity> credits(new CEntity(
        1, std::move(credits_moveable), std::move(credits_drawable)));
