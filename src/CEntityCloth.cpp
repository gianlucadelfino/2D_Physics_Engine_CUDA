      ...
      m_max_dist(10.0f)
{
    ...
    for (unsigned int i = 0; i < m_total_seams; ++i)
    {
        ...
        C2DVector pos(initial_pos_ +
                      m_max_dist * C2DVector(col * 1.0f, row * 1.0f));

        // add to the collection
        this->m_collection.push_back(
            std::shared_ptr<CEntityParticle>(new CEntityParticle(seam_)));
      ...
      m_max_dist(other_.m_max_dist)
{
    ...
    for (unsigned int i = 0; i < m_total_seams; ++i)
    {
        ...
        // add to the collection
        this->m_collection.push_back(
            std::shared_ptr<CEntityParticle>(new CEntityParticle(
                *other_.m_collection[i]))); // pass a copy of the particle
...
{
    for (std::vector<shared_ptr<CEntityParticle>>::iterator it =
             m_collection.begin();
         it != m_collection.end();
         ++it)
...
{
    // call update on all elements of c_collection, passing the collection total
    // force
    // this indirectly applies local constraints via the entity update
    for (std::vector<std::shared_ptr<CEntityParticle>>::iterator it =
             m_collection.begin();
         it != m_collection.end();
         ++it)
    {
...
void CEntityCloth::Draw() const
{
    ...
    for (unsigned int id = 0; id < this->m_collection.size(); ++id)
    {
        ...
        if (top_id >= 0)
        {
            C2DVector top_id_position =
                this->m_collection[top_id]->GetPosition();
            this->mp_drawable->Draw(top_id_position, id_pos);
        }

        if ((left_id % m_side_length) < (id % m_side_length))
        {
            C2DVector left_id_position =
                this->m_collection[left_id]->GetPosition();
